...
const VehicleHomeScreen: React.FC = () => {
  ...
  const loadVehicleData = async () => {
    ...
    try {
      ...
      // Sort maintenance logs by date (most recent first) for correct display order
  ...
  const renderMaintenanceTimeline = () => {
    ...
    // Use header prop instead of subtitle for interactive components
    const headerComponent = maintenanceLogs.length > 5 ? (
    ...
    return (
      <InfoCard
        title={headerComponent ? undefined : "Recent Maintenance Logs"}
            ...
            return (
              <View 
                ...
                style={[
                  ...
                  log.serviceType === 'shop' ? styles.timelineItemShop : styles.timelineItemDIY
                ]}
              >
                <View style={styles.timelineContent}>
                  ...
                  <View style={styles.timelineDetails}>
                    <Typography variant="bodySmall" style={styles.timelineDetailText}>
                  ...
                  {log.services && log.services.length > 0 && (
                    <View style={styles.timelineServicesContainer}>
                      <Typography variant="bodySmall" style={styles.timelineDetailText}>
                        Services:
                      </Typography>
                      {log.services.map((service, serviceIndex) => (
                        <Typography key={serviceIndex} variant="bodySmall" style={styles.timelineServiceItem}>
                          • {service.serviceName}
                        </Typography>
                      ))}
                    </View>
  ...
  const renderCostAnalytics = () => {
    ...
    // Analytics calculation temporarily removed

    return (
      <InfoCard
        title="Cost & Analytics"
        subtitle="Cost analytics will be displayed here once implemented"
        style={styles.sectionCard}
  ...
  const calculateCostAnalytics = (logs: MaintenanceLog[], vehicle: Vehicle | null) => {
    ...
    return {
      ...
      averagePerService: averagePerService || 0,
      recent30Days: recent30Days || 0,
      costPerMile: costPerMile ? parseFloat(costPerMile) : 0,
      timespan,
      trend,
      trendPercentage
    };
...
const styles = StyleSheet.create({
  ...
  timelineItemBorder: {
    ...
    marginBottom: theme.spacing.md, // Extra spacing between cards
  },
  timelineContent: {
    gap: theme.spacing.xs,
  },
